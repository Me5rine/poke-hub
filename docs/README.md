...
- **Collections (pool, sprites, recherche)** : alignement doc ↔ code dans [COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md) — mode **toutes les formes** vs **une entrée par espèce** (lignes `*-family`), exceptions formes catalogue (`switch_form` / `switch_battle`), synthèses **Dynamax / Gigamax** (pas de ligne hybride, pas de re-synthèse en chaîne), dimorphisme **collectionneur mâle/femelle** (`synthetic_sex_use_gender_asset`, base Dynamax synthétique), tri **Unown/Zarbi** (A→Z puis `!` puis `?`) ; helper images : `skip_gender_resolution` décrit dans [POKEMON_IMAGES.md](./POKEMON_IMAGES.md).
- **Import Game Master (référence)** : passes, lignes famille `*-family`, normalisation des protos de formes (dont cas **Galar / standard composé**), réécriture de la **`category`** des variants à chaque import, **Gigantamax** réservé à la fiche par défaut, lien avec les pools **collections** (costumes / copies). [pokemon/GAME_MASTER_IMPORT.md](./pokemon/GAME_MASTER_IMPORT.md).
