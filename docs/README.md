...
- **Collections (pool, sprites, recherche)** : alignement doc ↔ code dans [COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md) — mode **toutes les formes** vs **une entrée par espèce** (lignes `*-family`), exceptions formes catalogue (`switch_form` / `switch_battle`, dont Forces **641/642/645/905**), filtre **`poke_hub_collections_row_passes_pool_release_filter`** sur **`extra.release`** (incluant fiches **`gm_skeleton`** sans passe-partout), synthèses **Dynamax / Gigamax** (pas de ligne hybride, pas de re-synthèse en chaîne), dimorphisme **collectionneur mâle/femelle** (`synthetic_sex_use_gender_asset`, base Dynamax synthétique), tri **Unown/Zarbi** (A→Z puis `!` puis `?`) ; helper images : `skip_gender_resolution` décrit dans [POKEMON_IMAGES.md](./POKEMON_IMAGES.md).
