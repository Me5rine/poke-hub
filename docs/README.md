...
- **Form variants (formes)** : le **type de forme** (`category` : défaut, régional, fusion, changement de forme inventaire / combat, costume, clone, méga, forme visuelle, spéciale, shadow, purified, etc.) est choisi via un **menu déroulant** en admin ; l’import Game Master **pré-remplit** quand c’est possible (`poke_hub_pokemon_guess_form_type_from_gm`). Plus de champ **parent form** : le typage repose sur ce menu. Si la forme a la catégorie **Costume / event**, les Pokémon avec cette forme sont considérés costumés automatiquement. Voir [COLLECTIONS_AND_FORMS_CATEGORIES.md](./COLLECTIONS_AND_FORMS_CATEGORIES.md). Les **noms FR/EN** (`extra.names`) servent entre autres aux **préfixes** des phrases GO Collections — *Phrases GO* dans [COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md).
...
- **Collections (pool, sprites, recherche)** : alignement doc ↔ code dans [COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md) — mode **toutes les formes** vs **une entrée par espèce** (lignes `*-family`), **exception № 351 Morphéo / stub motifs `visual`** (`poke_hub_collections_visual_variant_base_stub_keep_dex_numbers`), **admin** **Poké HUB → Collections** (liste + progression + suppression), exceptions formes catalogue (`switch_form` / `switch_battle`, dont Forces **641/642/645/905**), filtre **`poke_hub_collections_row_passes_pool_release_filter`** sur **`extra.release`** (incluant fiches **`gm_skeleton`** sans passe-partout), synthèses **Dynamax / Gigamax** (pas de ligne hybride, pas de re-synthèse en chaîne), dimorphisme **collectionneur mâle/femelle** (`synthetic_sex_use_gender_asset`, base Dynamax synthétique), tri **Unown/Zarbi** (A→Z puis `!` puis `?`) ; helper images : `skip_gender_resolution` décrit dans [POKEMON_IMAGES.md](./POKEMON_IMAGES.md). Détail Morphéo + admin : [collections/CHANGELOG.md](./collections/CHANGELOG.md).
...
- **[COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md)** - Module Collections : shortcodes, pool, statuts (possédé / à l’échange / manquant), filtre **Include in grid** (chaîne source anglaise), options **genre** (`include_gender`, `show_gender_symbols`, `include_both_sexes_collector`), id synthétiques mâle/femelle, **ordre d’affichage des variantes** (`poke_hub_collections_sort_pool_display`), regroupement par génération (sans génération en **dernier**), vue **sticky / mode compact**, **options d’UI masquées selon la catégorie** (listes préparamétrées / cohérence pool, `poke_hub_collections_settings_hidden_control_keys`, `data-collections-control`, `pokeHubCollections.settingsHiddenByCategory`), bloc **phrases de recherche GO** (jeton **nom d’espèce**, **une entrée par dex**) ; styles et classes dans **modules/collections/COLLECTIONS_THEME_CSS.md** et **POKEHUB_CSS_CLASSES.md**
