...
- **Blocs Day Pokémon Hours (`featured_hours`)** : vignette **solo** même pour plusieurs Pokémon — **slug d’image cumulé** sur le bucket (ex. `pikachu-mimiqui.png`) ; noms sous la carte avec séparateurs **&**. **[CONTENT_BLOCKS.md](./CONTENT_BLOCKS.md)** (§ Day Pokémon Hours), rendu **`modules/blocks/blocks/day-pokemon-hours/render.php`**.
...
- **Icônes de types (CSS)** : `assets/css/poke-hub-type-icons.css` côté plugin ; **admin** toujours enfilé explicitement (`poke_hub_enqueue_admin_unified_styles` dans `poke-hub.php`) ; **front** via `poke_hub_register_bundled_front_style` seulement si le filtre front vaut `true`, sinon équivalent thème `parts/02-type-icons.css` — à garder alignés. Détail : [THEME_FRONT_CSS.md](./THEME_FRONT_CSS.md) (*Ce qui reste dans le plugin*), [INLINE_SVG.md](./INLINE_SVG.md).
...
- **Outils temporaires (admin)** : page **Poké HUB → Temporary tools** découpée en **onglets** — **Dates (Pokekalos)**, **Game Master**, **Translation** et **Images sync** (manifest CSV d’icônes GO, module Pokémon), **heures Fandom** / **Lundi Max** (module Events) ; option **Settings → General → Temporary tools** pour afficher ou masquer le sous-menu (`poke_hub_temporary_tools_enabled`). L’import Game Master et l’écran de traduction des **données** Pokémon ne sont plus sous **Settings**. Documentation : [ADMIN_TEMPORARY_TOOLS.md](./ADMIN_TEMPORARY_TOOLS.md) (**Images sync** : pas de filtrage par dates comme pour les collections ; imports Fandom : gabarits raid hour / vedette / Lundi Max ; onglets GM / Translation).
...
- **Catalogue bonus** : `title_en` / `title_fr`, un seul `slug` (fichier image), SVG prioritaire ; **bloc cartes** : description sous l’icône (sans afficher uniquement le nom du type comme libellé) ; **vignettes** : variables CSS `--pokehub-bonus-icon-*` (thème, pas d’option admin) — voir [BONUS_SOURCE_AND_BLOCKS.md](./BONUS_SOURCE_AND_BLOCKS.md).
...
- **Shop (admin + blocs)** : menu **Poké HUB → Shop** — onglets regroupés (Avatar shop : Items / Categories ; In‑game stickers) ; styles `.poke-hub-shop-items-nav*` dans `admin-unified.css` — [ADMIN_CSS.md](./ADMIN_CSS.md), module détaillé [shop-items/README.md](./shop-items/README.md). Blocs **Avatar shop highlights** / **In-game sticker highlights** : wrappers **`*-block-wrapper`** (cadre titre comme les autres blocs), accroche + **`pokehub-block-subtitle`**, vignettes grille sans carré forcé pour noms longs — `parts/04-blocks-front.css` (thème Me5rine) ; metaboxes dans `modules/blocks/admin/` — [CONTENT_BLOCKS.md](./CONTENT_BLOCKS.md), [POKEHUB_CSS_CLASSES.md](./POKEHUB_CSS_CLASSES.md).
...
- **Form variants (formes)** : le **type de forme** (`category` : défaut, régional, fusion, changement de forme inventaire / combat, costume, clone, méga, forme visuelle, spéciale, shadow, purified, etc.) est choisi via un **menu déroulant** en admin ; l’import Game Master **pré-remplit** quand c’est possible (`poke_hub_pokemon_guess_form_type_from_gm`). Plus de champ **parent form** : le typage repose sur ce menu. Si la forme a la catégorie **Costume / event**, les Pokémon avec cette forme sont considérés costumés automatiquement. Voir [COLLECTIONS_AND_FORMS_CATEGORIES.md](./COLLECTIONS_AND_FORMS_CATEGORIES.md). Les **noms FR/EN** (`extra.names`) servent entre autres aux **préfixes de variante** (`pogo_group_prefix_*`) des phrases GO ; le reste des filtres (chromatique, événement, fonds, sexe, chanceux, bébés…) est documenté dans [COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md) (*Phrases GO : données serveur et langue*).
...
- **Collections (pool, sprites, recherche)** : alignement doc ↔ code dans [COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md) — persistance **anonymes** (`user_id = 0`, jeton / cookie propriétaire, voir § *Stockage*), mode **toutes les formes** vs **une entrée par espèce** (lignes `*-family`), **exception № 351 Morphéo / stub motifs `visual`** (`poke_hub_collections_visual_variant_base_stub_keep_dex_numbers`), **admin** **Poké HUB → Collections** (liste + progression + suppression), **tri d’affichage du pool** (`poke_hub_collections_display_variant_sort_rank` : costumes avant variante « régionale » / assimilée, interclassement **méga / gigamax / dynamax** aligné sur les sélecteurs), exceptions formes catalogue (`switch_form` / `switch_battle`, dont Forces **641/642/645/905**), filtre **`poke_hub_collections_row_passes_pool_release_filter`** sur **`extra.release`** (incluant fiches **`gm_skeleton`** sans passe-partout), synthèses **Dynamax / Gigamax** (pas de ligne hybride, pas de re-synthèse en chaîne), dimorphisme **collectionneur mâle/femelle** (`synthetic_sex_use_gender_asset`, base Dynamax synthétique), tri **Unown/Zarbi** (A→Z puis `!` puis `?`), slugs fiche Zarbi après import : **`poke_hub_pokemon_compose_pokemon_row_slug`** — [GAME_MASTER_IMPORT.md](./pokemon/GAME_MASTER_IMPORT.md) § *Slug ligne Pokémon* ; helper images : `skip_gender_resolution` dans [POKEMON_IMAGES.md](./POKEMON_IMAGES.md). Détail Morphéo + admin + UI barre fixe : [collections/CHANGELOG.md](./collections/CHANGELOG.md).
- **Import Game Master (référence)** : passes, lignes famille `*-family`, normalisation des protos de formes (dont cas **Galar / standard composé**), **alias slug granular → registre** pour `pokemon_form_variants`, réécriture de la **`category`** des variants à chaque import, **Gigantamax** réservé à la fiche par défaut, lien avec les pools **collections** (costumes / copies). [pokemon/GAME_MASTER_IMPORT.md](./pokemon/GAME_MASTER_IMPORT.md).
- **Sécurité des données Pokémon** : import Game Master (**Temporary tools → Game Master**) — fusion **profonde** de `extra`, préservation des noms i18n et de certains flags manuels, liaisons **non destructives** (types / attaques / attaque–type), **UPDATE SQL** limité aux colonnes modifiées, Méga (formats `$wpdb` + repli slug). Détail : [pokemon/DATA_SAFETY.md](./pokemon/DATA_SAFETY.md).
...
- **[ADMIN_TEMPORARY_TOOLS.md](./ADMIN_TEMPORARY_TOOLS.md)** - Page *Temporary tools* (onglets : Pokekalos, **Game Master**, **Translation**, **Images sync**, Fandom… ; option d’activation ; **§ Images sync**, **§ Imports Fandom**)
...
- **[COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md)** - Module Collections : shortcodes, pool, statuts (possédé / à l’échange / manquant), stockage connecté vs **anonyme en base**, filtre **Include in grid** (chaîne source anglaise), options **genre** (`include_gender`, `show_gender_symbols`, `include_both_sexes_collector`), id synthétiques mâle/femelle, **ordre d’affichage des variantes** (`poke_hub_collections_sort_pool_display`, `poke_hub_collections_display_variant_sort_rank`), regroupement par génération (sans génération en **dernier**), **barre d’outils fixe** et **menu tiroir** (pas de « mode compact » historique), **options d’UI masquées selon la catégorie** (listes préparamétrées / cohérence pool, `poke_hub_collections_settings_hidden_control_keys`, `data-collections-control`, `pokeHubCollections.settingsHiddenByCategory`), bloc **phrases de recherche GO** (préfixes FR/EN, regroupements, dédoublonnage parent/sexe, fonds lieu/spécial, contexte chanceux/bébés ; jeton **nom d’espèce**) ; styles et classes dans **modules/collections/COLLECTIONS_THEME_CSS.md** et **POKEHUB_CSS_CLASSES.md**
...
- **[pokemon/](./pokemon/)** - Module Pokémon
  - [README.md](./pokemon/README.md) - Index (biomes, import GM, sécurité données, régional, Pokekalos)
  - [GAME_MASTER_IMPORT.md](./pokemon/GAME_MASTER_IMPORT.md) - Import Game Master : passes, slug / familles / variants / Gigamax
...
- **[collections/](./collections/)** - Module Pokémon GO Collections
  - [README.md](./collections/README.md) - Index vers [COLLECTIONS_MODULE.md](./COLLECTIONS_MODULE.md), [CHANGELOG.md](./collections/CHANGELOG.md), formes vs collections, CSS thème
...
- **[user-profiles/](./user-profiles/)** - Module User Profiles
  ...
  - [FRIEND_CODES_PUBLIC_AND_IP.md](./user-profiles/FRIEND_CODES_PUBLIC_AND_IP.md) - Codes amis, visibilité publique, IP, visiteurs
