...
- **Collections (vue grille)** : libellés du filtre d’affichage et du message d’aide dans `modules/collections/public/collections-shortcode.php` (sources anglaises actuelles, ex. **Include in grid**, *Click a tile to cycle status.*, *Owned*, *For trade*, *Missing*, *Select at least one status…*). Anciennes formulations documentées (« *Show in grid* », « Afficher dans la grille ») peuvent encore apparaître dans d’anciens guides ; le fichier de vérité de traduction pour le comportement fonctionnel reste **docs/COLLECTIONS_MODULE.md** (section *Statuts d’une entrée* et paragraphe sur le bloc *Include in grid*).
...
- **Shop highlights (front) :** `modules/blocks/blocks/shop-avatar-highlights/render.php`, `shop-sticker-highlights/render.php` — phrases d’accroche, sous-titres des tuiles (`h3` et paragraphe d’intro portent **`pokehub-block-subtitle`** en plus des classes métier), libellés d’accessibilité ; helpers `pokehub_shop_highlights_resolve_event_label()` / `pokehub_shop_highlights_collect_item_display_names()` dans `modules/blocks/functions/blocks-helpers.php` (chaîne de repli `this event`, etc.)
- **Day Pokémon Hours (`featured_hours`, multi-Pokémon) :** liste de noms sous la tuile avec `sprintf(__(' %1$s & %2$s ', 'poke-hub'), …)` — la traduction FR peut rendre **`%1$s et %2$s`** pour un libellé correct en français.
