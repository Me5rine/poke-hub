...
6. **Formes / variantes globales** : à l’import, réutilisation d’une ligne existante si le **slug** de forme est déjà connu (`poke_hub_pokemon_get_form_variant_by_slug` / `poke_hub_pokemon_upsert_form_variant`) ; sinon création. Le **type de forme** (`category`) est **recalculé** à chaque import à partir du GM (`poke_hub_pokemon_guess_form_type_from_gm`) puis reste éditable en admin ; la catégorie n’est pas figée après la première création. Synthèse des règles slug famille / Proto : [GAME_MASTER_IMPORT.md](./GAME_MASTER_IMPORT.md).
