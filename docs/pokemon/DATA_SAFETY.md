...
6. **Formes / variantes globales** : le slug **granulaire** (proto GM normalisé) sert au **suffixe slug Pokémon** et à **`extra.form_slug`**. Pour la table **`pokemon_form_variants`**, un **slug de registre** peut différer : textarea **Variant registry slug aliases** (option **`poke_hub_gm_variant_registry_slug_aliases_lines`**), filtres **`poke_hub_gm_variant_registry_slug_aliases_map`** et **`poke_hub_gm_variant_registry_slug`**, fonction **`poke_hub_resolve_gm_variant_registry_slug()`** — voir § *Variantes globales* dans [GAME_MASTER_IMPORT.md](./GAME_MASTER_IMPORT.md). L’upsert (**`poke_hub_pokemon_get_form_variant_by_slug`** / **`poke_hub_pokemon_upsert_form_variant`**) cible ce slug de registre. Le **type de forme** (`category`) est **recalculé** à chaque import (**`poke_hub_pokemon_guess_form_type_from_gm`**), puis reste éditable en admin. Le **label** peut être figé avec **`extra.manual_variant_label`** sur une variante. Les slugs volontairement exclus après suppression admin peuvent être listés (option **`poke_hub_gm_suppressed_form_slugs`**, filtre **`poke_hub_gm_suppressed_form_slugs`**), avec une garde fondée sur le **slug granular** lors de l’import même si un alias utilise un registre différent ; la sauvegarde depuis le formulaire admin désactive cette garde. Synthèse détaillée : [GAME_MASTER_IMPORT.md](./GAME_MASTER_IMPORT.md) (dont slug famille / protos).
...
8. **Gigantamax** : détection via `sourdoughMoveMappingSettings` et moves `VN_BM_*` ; `extra['has_gmax_form']` est porté uniquement comme indicateur métier pertinent sur la **fiche espèce « par défaut »** (`is_default = 1`) ; après fusion `extra`, la valeur peut être réécrite selon ce rôle ; liaisons d’attaques en rôle **`gmax`** ; case manuelle possible sur la fiche Pokémon. Détail : section Gigantamax dans [GAME_MASTER_IMPORT.md](./GAME_MASTER_IMPORT.md).
...
## Évolutions Game Master (`pokemon_evolutions.extra`)

Pour les lignes synchronisées depuis le **PASS 3** de l’import GM, **`extra`** peut contenir **`evolution_source = game_master`** et les protos **`base_id_proto`**, **`target_id_proto`**, **`target_form_proto`** (voir [GAME_MASTER_IMPORT.md](./GAME_MASTER_IMPORT.md)). Cela documente **l’intention** du fichier GM indépendamment des **`pokemon.id`** après coup et sert aux garde-fous d’affichage côté blocs lorsque les liaisons anciennes ne correspondent plus aux fiches réelles.

**Placeholders `*-family` :** depuis la résolution **`poke_hub_pokemon_gm_resolve_index_row_for_evolution_links()`**, les nouvelles écritures PASS 3 rattachent en principe les arêtes aux **fiches non famille** lorsqu’elles existent pour le proto. Une base de données ancienne peut encore contenir des **`base_pokemon_id` / `target_pokemon_id`** pointant uniquement vers une ligne **slug `*-family`** ; un **import GM complet** (ou une resynchronisation des espèces concernées) aide à régénérer les bons couples base/cible. L’affichage front (**`pokehub_get_pokemon_evolutions_in` / `_out`**) peut en parallèle **masquer** les arêtes impliquant un slug `*-family` même si une vieille ligne de liaison existe encore — voir **§ Hors import — où le slug `*-family` est ignoré métier** dans [GAME_MASTER_IMPORT.md](./GAME_MASTER_IMPORT.md).

## Suppression admin des fiches Pokémon (liaisons)

Avant **`DELETE`** sur **`pokemon`**, l’admin appelle **`poke_hub_pokemon_delete_linked_rows_for_pokemon_ids()`** :

- **`pokemon_attack_links`**, **`pokemon_type_links`**, **`pokemon_biome_pokemon_links`** : suppression des lignes où **`pokemon_id`** figure dans l’ensemble d’IDs.
