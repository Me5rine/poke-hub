...
|----------|-------------------|
| [GAME_MASTER_IMPORT.md](./GAME_MASTER_IMPORT.md) | Import GM : passes, génération canonique (9 générations), région d’origine GO (`origin_region_id` / `origin_region_slug` avec fallback via génération), PASS 3 `pokemon_evolutions` (résolution hors `*-family`, `extra` protos / `evolution_source`), placeholders `*-family`, Gigamax, variants (`category`, **Forces / Amovénus** avant `ibfc`, alias granular → registre), squelettes **`gm_skeleton`**, liens Collections |
