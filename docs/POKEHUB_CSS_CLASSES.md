...
- `.pokehub-collections-drawer`, `.pokehub-collections-drawer-*` — panneau latéral (header = `me5rine-lab-card-header` + `me5rine-lab-title-medium`) ; variante **`.pokehub-collections-drawer--toolbar`** + **`[data-toolbar-menu-drawer]`** pour le menu des sections barre d’outils (petit écran) — **`position: fixed`** plein viewport dans le thème ; le **HTML du plugin** place ce bloc **après** **`.pokehub-collection-toolbar-stack`**, pas à l’intérieur (**COLLECTIONS_MODULE.md**, § *Vue détail : structure DOM*).
...
- **Thème Me5rine Lab (source de vérité front)** : chaque `css/poke-hub/parts/*.css` (événements, bonus, blocs, collections, profils, etc.) est enqueued individuellement par le `functions.php` du thème (`glob` + tri naturel) ; `css/poke-hub/poke-hub-late-overrides.css` ferme la cascade. Le fichier `poke-hub-front.css` reste un **index `@import`** pour la lisibilité et `add_editor_style()`, **pas** un bundle utilisé en front navigateur. Détail : **[THEME_FRONT_CSS.md](./THEME_FRONT_CSS.md)**.
