...
**Cohérence BDD et filtre GM** : la résolution des évolutions entrantes utilise une jointure avec la **fiche `pokemon` cible** incluant **`target_form_variant_id`** (avec `COALESCE(..., 0)`, comme les filtres admin), afin de ne pas mélanger des variantes différentes. Les placeholders **slug `*-family`** sont exclus des jointures d’affichage (pas de nœud d’évolution affiché pour ces lignes BDD lorsqu’elles apparaissaient encore dans `pokemon_evolutions`). Pour les lignes dont **`extra.evolution_source`** vaut **`game_master`**, un filtre optionnel (**`pokehub_filter_evolutions_by_game_master_proto`**, activé par défaut) conserve uniquement les arêtes dont les protos en `extra` concordent avec **`extra.pokemon_id_proto`** des fiches base et cible — limitation des artefacts si les **`pokemon_id`** ont divergé de l’historique GM. Synthèse import + comportement hors collections : **[docs/pokemon/GAME_MASTER_IMPORT.md](pokemon/GAME_MASTER_IMPORT.md)**. Détail technique : **§ Bloc New Pokémon – Evolution Lines** dans [blocks/BLOCK_STYLES_AND_BEHAVIOR.md](blocks/BLOCK_STYLES_AND_BEHAVIOR.md).
