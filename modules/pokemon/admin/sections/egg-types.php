<?php
// modules/pokemon/admin/sections/egg-types.php
...
require_once POKE_HUB_POKEMON_PATH . '/admin/forms/egg-type-form.php';

class Poke_Hub_Pokemon_Egg_Types_List_Table extends WP_List_Table {

    public function __construct() {
        parent::__construct([
            'singular' => 'pokemon_egg_type',
            'plural'   => 'pokemon_egg_types',
            'ajax'     => false,
        ]);
    }
    ...
    public function get_columns() {
        return [
            ...
            'hatch_distance' => __('Hatch distance', 'poke-hub'),
            'image_url'    => __('Icon', 'poke-hub'),
        ];
    }
    ...
    public function get_sortable_columns() {
        return [
            'name' => ['name_fr', true],
            'slug' => ['slug', true],
            'hatch_distance' => ['hatch_distance_km', true],
        ];
    }
    ...
    public function column_name($item) {
        ...
        $delete_url = wp_nonce_url(
            ...
            'poke_hub_delete_egg_type_' . (int) $item->id
        );
        ...
        $actions['delete'] = sprintf(
            ...
            esc_url($delete_url),
            esc_attr__('Are you sure you want to delete this egg type?', 'poke-hub'),
    ...
    public function column_hatch_distance($item) {
        $km = isset($item->hatch_distance_km) ? (int) $item->hatch_distance_km : 0;
        return $km . ' km';
    }
    ...
    public function prepare_items() {
        ...
        $allowed_orderby = ['name_fr', 'name_en', 'slug', 'hatch_distance_km'];
...
/**
 * Traitement formulaire Egg Type (add / update)
 */
function poke_hub_pokemon_handle_egg_types_form() {
    if (!is_admin()) {
        return;
    }
    ...
    if (empty($_POST['ph_section']) || $_POST['ph_section'] !== 'egg_types') {
        return;
    }

    $action = sanitize_text_field($_POST['poke_hub_pokemon_action']);
    if (!in_array($action, ['add_egg_type', 'update_egg_type'], true)) {
        return;
    }
    ...
    check_admin_referer('poke_hub_pokemon_edit_egg_type');
    ...
    $hatch_km   = isset($_POST['hatch_distance_km']) ? max(0, (int) $_POST['hatch_distance_km']) : 2;
    $image_url  = isset($_POST['image_url']) ? esc_url_raw(wp_unslash($_POST['image_url'])) : '';
    $is_adventure_sync = !empty($_POST['is_adventure_sync']);
    ...
    $data = [
        ...
        'name_fr'            => $name_fr,
        'name_en'            => $name_en,
        'hatch_distance_km'   => $hatch_km,
        'extra'              => wp_json_encode($extra),
    ];
    $format = ['%s', '%s', '%s', '%d', '%s'];

    if ($action === 'add_egg_type') {
...
add_action('admin_init', 'poke_hub_pokemon_handle_egg_types_form');

/**
 * Delete simple (action=delete sur une ligne)
 */
function poke_hub_pokemon_handle_egg_types_delete() {
    if (!is_admin()) {
        return;
    }
    ...
    if (empty($_GET['ph_section']) || $_GET['ph_section'] !== 'egg_types') {
        return;
    }
    ...
    check_admin_referer('poke_hub_delete_egg_type_' . $id);
...
add_action('admin_init', 'poke_hub_pokemon_handle_egg_types_delete');

/**
 * Écran principal "Egg types"
 */
function poke_hub_pokemon_admin_egg_types_screen() {
    $list_table = new Poke_Hub_Pokemon_Egg_Types_List_Table();
    ...
    if (!empty($_GET['ph_msg'])) {
        ...
        if ($msg === 'saved' || $msg === 'updated') {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Egg type saved.', 'poke-hub') . '</p></div>';
        } elseif ($msg === 'deleted') {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Egg type deleted.', 'poke-hub') . '</p></div>';
            ...
            echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__('Invalid egg type ID.', 'poke-hub') . '</p></div>';
        }
    }
    ?>
    <form method="post">
        ...
        <?php
        wp_nonce_field('bulk-pokemon_egg_types');
        $list_table->search_box(__('Search egg type', 'poke-hub'), 'pokemon-egg-type');
        $list_table->display();
        ?>
    </form>
    <?php
