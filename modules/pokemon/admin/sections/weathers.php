<?php
// modules/pokemon/admin/sections/weathers.php
...
// Formulaire add/edit
require_once POKE_HUB_POKEMON_PATH . '/admin/forms/weather-form.php';

class Poke_Hub_Pokemon_Weathers_List_Table extends WP_List_Table {

    public function __construct() {
        parent::__construct([
            'singular' => 'pokemon_weather',
            'plural'   => 'pokemon_weathers',
            'ajax'     => false,
        ]);
    }
    ...
    public function get_columns() {
        return [
            ...
            // On garde "name" comme ID de colonne pour WP_List_Table,
            // mais derrière on affichera name_fr / name_en.
    ...
    public function get_sortable_columns() {
        return [
            // On trie réellement sur name_fr en base.
            'name' => ['name_fr', true],
            'slug' => ['slug', true],
        ];
    }
    ...
    public function column_name($item) {
        ...
        $delete_url = wp_nonce_url(
            ...
            'poke_hub_delete_weather_' . (int) $item->id
        );

        // On choisit le nom à afficher : priorité au FR, sinon EN.
        ...
        // URL de la page publique de la météo
        $view_url = '';
        if (!empty($item->slug) && function_exists('pokehub_get_weather_url')) {
            $view_url = pokehub_get_weather_url($item->slug);
        }
        ...
        $actions['delete'] = sprintf(
            ...
            esc_url($delete_url),
            esc_attr__('Are you sure you want to delete this weather?', 'poke-hub'),
    ...
    public function process_bulk_action() {
        ...
        check_admin_referer('bulk-pokemon_weathers');
    ...
    public function prepare_items() {
        ...
        // Colonnes de tri autorisées en base
        ...
        if ($search !== '') {
            // Recherche sur name_en, name_fr et slug
...
/**
 * Traitement formulaire Weather (add / update)
 */
function poke_hub_pokemon_handle_weathers_form() {
    if (!is_admin()) {
        return;
    }
    ...
    if (empty($_POST['ph_section']) || $_POST['ph_section'] !== 'weathers') {
        return;
    }

    $action = sanitize_text_field($_POST['poke_hub_pokemon_action']);
    if (!in_array($action, ['add_weather', 'update_weather'], true)) {
        return;
    }
    ...
    check_admin_referer('poke_hub_pokemon_edit_weather');
...
add_action('admin_init', 'poke_hub_pokemon_handle_weathers_form');

/**
 * Delete simple (action=delete sur une ligne)
 */
function poke_hub_pokemon_handle_weathers_delete() {
    if (!is_admin()) {
        return;
    }
    ...
    if (empty($_GET['ph_section']) || $_GET['ph_section'] !== 'weathers') {
        return;
    }
    ...
    check_admin_referer('poke_hub_delete_weather_' . $id);
...
add_action('admin_init', 'poke_hub_pokemon_handle_weathers_delete');

/**
 * Écran principal "Weathers"
 */
function poke_hub_pokemon_admin_weathers_screen() {
    $list_table = new Poke_Hub_Pokemon_Weathers_List_Table();
    ...
    if (!empty($_GET['ph_msg'])) {
        ...
        if ($msg === 'saved' || $msg === 'updated') {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Weather saved.', 'poke-hub') . '</p></div>';
        } elseif ($msg === 'deleted') {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Weather deleted.', 'poke-hub') . '</p></div>';
            ...
            echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__('Invalid weather ID.', 'poke-hub') . '</p></div>';
        }
    }
    ?>
    <form method="post">
        ...
        wp_nonce_field('bulk-pokemon_weathers');

        $list_table->search_box(__('Search weather', 'poke-hub'), 'pokemon-weather');
        $list_table->display();
        ?>
    </form>
    <?php
