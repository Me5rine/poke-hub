<?php
/**
 * AJAX formulaire admin boutique avatar (liaisons événements) — module shop-items.
 */
...
if (defined('POKEHUB_SHOP_AVATAR_ITEM_FORM_AJAX_LOADED')) {
    return;
}
define('POKEHUB_SHOP_AVATAR_ITEM_FORM_AJAX_LOADED', true);

add_action('wp_ajax_pokehub_shop_avatar_special_events_search', function (): void {
    if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'pokehub_shop_avatar_item_form_ajax')) {
    ...
    foreach ((array) $rows as $row) {
        $out[] = [
            'id'   => (int) $row->id,
            'text' => function_exists('poke_hub_shop_avatar_special_event_label')
                ? poke_hub_shop_avatar_special_event_label($row)
                : (string) ($row->id ?? ''),
        ];
    }
