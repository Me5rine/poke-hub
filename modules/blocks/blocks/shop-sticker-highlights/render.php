<?php
/**
 * Bloc « In-game sticker highlights »
 *
...
if (!function_exists('pokehub_content_get_shop_sticker') || !function_exists('poke_hub_shop_sticker_get_items_by_ids')) {
    return '';
}

$data = pokehub_content_get_shop_sticker('post', $post_id);
...
$items_by_id = !empty($item_ids) ? poke_hub_shop_sticker_get_items_by_ids($item_ids) : [];
$ordered_items = [];
...
if ($has_items && function_exists('pokehub_shop_highlights_resolve_event_label') && function_exists('pokehub_shop_highlights_collect_item_display_names')) {
    ...
    if (!empty($raw_names)) {
        ...
        $intro_html   = sprintf(
            /* translators: 1: comma-separated sticker names, 2: event name. */
            __('The stickers %1$s will be available in the shop, at PokéStops, and in Gifts during the event "%2$s".', 'poke-hub'),
            $names_joined,
...
$wrapper_attributes = get_block_wrapper_attributes([
    'class' => 'pokehub-shop-sticker-highlights-block-wrapper pokehub-shop-sticker-highlights-block pokehub-shop-highlights',
]);
...
<div <?php echo $wrapper_attributes; ?>>
    <?php echo function_exists('pokehub_render_block_title')
        ? pokehub_render_block_title(__('New stickers in the shop', 'poke-hub'), 'shop-sticker-highlights')
        : '<h2 class="pokehub-block-title">' . esc_html__('New stickers in the shop', 'poke-hub') . '</h2>'; ?>
        ...
        <div class="pokehub-shop-highlights-panel">
            <?php if ($show_lead) : ?>
                <div class="pokehub-shop-highlights-lead">
                    <?php if ($has_hero) : ?>
                        <figure class="pokehub-shop-highlights-lead__figure pokehub-shop-highlights-hero pokehub-shop-highlights-hero--sticker" aria-label="<?php esc_attr_e('Cover image', 'poke-hub'); ?>">
                            <?php
            ...
            <?php if ($has_items) : ?>
                <div class="pokehub-shop-highlights-panel__tiles">
                    <h3 class="pokehub-shop-highlights-panel__tiles-title pokehub-block-subtitle"><?php esc_html_e('Stickers in this event', 'poke-hub'); ?></h3>
                    <div class="pokehub-wild-pokemon-grid pokehub-shop-highlights-items pokehub-shop-sticker-items-grid">
                        <?php foreach ($ordered_items as $row) : ?>
                            <?php
                            $urls = poke_hub_shop_sticker_get_item_image_urls($row);
                            $display = trim((string) $row->name_fr) !== '' ? (string) $row->name_fr : (string) $row->name_en;
                            if ($display === '') {
                                $display = (string) $row->slug;
                            }
                            ?>
                            <div class="pokehub-wild-pokemon-card pokehub-shop-sticker-item-card">
