(function ($) {
    ...
    function init() {
        var c = window.pokehubShopAvatarMetabox;
        if (!c || !c.ajaxUrl || !$.fn.select2) {
            return;
        }

        var $sel = $('#pokehub_shop_avatar_item_ids');
        if (!$sel.length) {
            return;
        }
        ...
        $sel.select2({
            ...
            ajax: {
                ...
                data: function (params) {
                    return {
                        action: 'pokehub_shop_avatar_items_search',
                        nonce: c.nonce,
                        q: params.term || ''
                    };
                },
        ...
        var $btnCreate = $('#pokehub-shop-avatar-create-submit');
        var $inEn = $('#pokehub-shop-avatar-create-name-en');
        var $inFr = $('#pokehub-shop-avatar-create-name-fr');
        var $hint = $('#pokehub-shop-avatar-create-inline-hint');
        var hintTimer;
        ...
        if ($btnCreate.length) {
            $btnCreate.on('click', function () {
                ...
                $.post(c.ajaxUrl, {
                    action: 'pokehub_shop_avatar_metabox_create_item',
                    nonce: c.nonce,
                    post_id: postId,
                    name_en: nameEn,
                    name_fr: nameFr
        ...
        var frame;
        var $heroId = $('#pokehub_shop_avatar_hero_id');
        var $prevWrap = $('#pokehub-shop-avatar-hero-preview-wrap');
        var $prevImg = $('#pokehub-shop-avatar-hero-preview');
        var $btnClear = $('#pokehub-shop-avatar-hero-clear');

        $('#pokehub-shop-avatar-hero-pick').on('click', function (e) {
            e.preventDefault();
            if (frame) {
                frame.open();
                return;
