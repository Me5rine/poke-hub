    ...
    if (function_exists('poke_hub_pokemon_get_regional_mappings_from_db')) {
        ...
        if (!empty($mappings_from_table)) {
            ...
            if (!empty($mapping) && count($missing_patterns) <= 3) {
                ...
                if (defined('WP_DEBUG') && WP_DEBUG && !empty($mapping)) {
                    error_log('[PokeHub] Vivillon mapping from DB incomplete. Found ' . count($found_patterns) . ' patterns, missing: ' . implode(', ', $missing_patterns));
                }
            }
        }
    }
